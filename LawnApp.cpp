...
void LawnApp::StartDiscord()
{
	...
	Discord_Initialize("1243863455052595230", &handlers, 1, NULL);
	UpdateDiscordRPC();
}

void LawnApp::UpdateDiscordRPC(const std::string Details, const std::string State, const std::string ImageLarge, const std::string ImageSmall)
{
	DiscordRichPresence discordPresence;
	memset(&discordPresence, 0, sizeof(discordPresence));
	discordPresence.state = State.c_str();
	discordPresence.details = Details.c_str();
	discordPresence.largeImageKey = ImageLarge.c_str();
	discordPresence.smallImageKey = ImageSmall.c_str();
...
{
	if (!mTodCheatKeys && CanPauseNow() && mAutoPause)
	{
		DoPauseDialog();
	}
}
...
void LawnApp::DoBackToMainNotSilent()
{
	mSoundSystem->CancelPausedFoley();
	WriteCurrentUserConfig();
	KillNewOptionsDialog();
...
bool LawnApp::KillNewOptionsDialog()
{
	...
	mFake3DAcceleration = want3D;
	mHQMusic = aNewOptionsDialog->mHQMusicCheckbox->IsChecked();
...
{
	if (mBoard == nullptr) {
		UpdateDiscordRPC(TodStringTranslate(_S("[MAIN_MENU]")), TodStringTranslate(_S("[MAIN_MENU_RICH_PRESENCE]")));
	}
...
void LawnApp::Init()
{
	...
	if (!Is3DAccelerationSupported() || !Is3DAccelerationRecommended())
	{
		mFake3DAcceleration = false;
		//RegistryEraseValue(_S("Fake3D"));
	}
...
bool LawnApp::UpdatePlayerProfileForFinishingLevel()
{
	...
	else if (IsPuzzleMode())
	{
		...
		{
			mPlayerInfo->mChallengeVeteranRecords[GetCurrentChallengeIndex()]++;
		}
		else
		{
	...
	{
		aUnlockedNewChallenge = (mBoard->mIsVeteran ? !HasBeatenChallengeVeteran(mGameMode) : !HasBeatenChallenge(mGameMode));
		if (mBoard->mIsVeteran)
		{
			mPlayerInfo->mChallengeVeteranRecords[GetCurrentChallengeIndex()]++;
		}
...
{
	return mGameMode >= GameMode::GAMEMODE_SURVIVAL_NORMAL_STAGE_1 && mGameMode <= GameMode::GAMEMODE_SURVIVAL_ENDLESS_STAGE_6 ||
		mGameMode >= GameMode::GAMEMODE_COOP_SURVIVAL_NORMAL_STAGE_1 && mGameMode <= GameMode::GAMEMODE_COOP_SURVIVAL_ENDLESS_STAGE_6;
}
...
bool LawnApp::IsSurvivalNormal(GameMode theGameMode)
{
	...
	int aLevel2 = theGameMode - GameMode::GAMEMODE_COOP_SURVIVAL_NORMAL_STAGE_1;
	return aLevel >= 0 && aLevel <= 5 || aLevel2 >= 0 && aLevel2 <= 5;
}

bool LawnApp::IsSurvivalHard(GameMode theGameMode)
{
	int aLevel = theGameMode - GameMode::GAMEMODE_SURVIVAL_HARD_STAGE_1;
	int aLevel2 = theGameMode - GameMode::GAMEMODE_COOP_SURVIVAL_HARD_STAGE_1;
	return aLevel >= 0 && aLevel <= 5 || aLevel2 >= 0 && aLevel2 <= 5;
}
...
bool LawnApp::IsSurvivalEndless(GameMode theGameMode)
{
	...
	int aLevel2 = theGameMode - GameMode::GAMEMODE_COOP_SURVIVAL_ENDLESS_STAGE_1;
	return aLevel >= 0 && aLevel <= 5 || aLevel2 >= 0 && aLevel2 <= 5;
}
...
bool LawnApp::IsContinuousChallenge()
{
	return
		IsArtChallenge() ||
		IsSlotMachineLevel() ||
		mGameMode == GAMEMODE_CHALLENGE_RENEWABLE_ENERGY ||
		mGameMode == GAMEMODE_CHALLENGE_AIR_RAID ||
		IsFinalBossLevel() ||
		mGameMode == GameMode::GAMEMODE_CHALLENGE_GRAVEYARD_ENCOUNTER ||
...
bool LawnApp::HasSecondarySeedBank()
{
	return mGameMode >= GAMEMODE_COOP_SURVIVAL_NORMAL_STAGE_1 && mGameMode <= GAMEMODE_COOP_SURVIVAL_ENDLESS_STAGE_6 ||
		   mGameMode == GAMEMODE_VERSUS_PLANTS_VS_PLANTS;
}
...
bool LawnApp::IsArtChallenge()
{
	...
	return
		mGameMode == GameMode::GAMEMODE_CHALLENGE_ART_SEEING_NUTS ||
		mGameMode == GameMode::GAMEMODE_CHALLENGE_SEEING_FLOWERS ||
...
{
	return mBoard && mGameMode == GameMode::GAMEMODE_CHALLENGE_CAN_YOU_DIG_IT;
}

//0x453840
bool LawnApp::IsWallnutBowlingLevel()
{
	if (mGameMode == GameMode::GAMEMODE_CHALLENGE_WALLNUT_BOWLING  || mGameMode == GameMode::GAMEMODE_CHALLENGE_WALLNUT_BOWLING_2 || mGameMode == GAMEMODE_CHALLENGE_WALLNUT_BOWLING_3)
		return true;
...
{
	return (mBoard && (mGameMode == GameMode::GAMEMODE_CHALLENGE_SLOT_MACHINE || mGameMode == GameMode::GAMEMODE_CHALLENGE_SLOT_MACHINE_2));
}
...
{
	return mBoard && mGameMode == GameMode::GAMEMODE_CHALLENGE_BIG_TROUBLE;
}

bool LawnApp::IsLastStandLevel(GameMode theGameMode)
{
	int aLevel = theGameMode - GameMode::GAMEMODE_CHALLENGE_LAST_STAND_DAY;
...
bool LawnApp::IsStormyNightLevel()
{
	...
	if (mGameMode == GameMode::GAMEMODE_CHALLENGE_DARK_STORMY_NIGHT || mGameMode == GameMode::GAMEMODE_CHALLENGE_STORMY_NIGHT_2)
		return true;
...
bool LawnApp::IsFinalBossLevel()
{
	...
	if (mGameMode == GameMode::GAMEMODE_CHALLENGE_DR_ZOMBOSS_REVENGE)
		return true;
...
int LawnApp::GetSeedsAvailable()
{
	...
	if (HasFinishedAdventure() || aLevel > 50 || mGameMode == GAMEMODE_VERSUS_PLANTS_VS_PLANTS)
	{
...
bool LawnApp::HasSeedType(SeedType theSeedType)
{
	...
	*/
	...
	if (theSeedType == SeedType::SEED_DUMMY)
	{
		return mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_PLANT_EXPENSIVE_NUT] > 0;
	}
...
bool LawnApp::HasBeatenChallengeVeteran(GameMode theGameMode)
{
	if (mPlayerInfo == nullptr)
		return false;
	...
	if (IsSurvivalNormal(theGameMode))
	{
		return mPlayerInfo->mChallengeVeteranRecords[aChallengeIndex] >= SURVIVAL_NORMAL_FLAGS;
	}
	if (IsSurvivalHard(theGameMode))
	{
		return mPlayerInfo->mChallengeVeteranRecords[aChallengeIndex] >= SURVIVAL_HARD_FLAGS;
	}
	if (IsSurvivalEndless(theGameMode) || IsEndlessScaryPotter(theGameMode) || IsEndlessIZombie(theGameMode))
	{
		return false;
	}
	return mPlayerInfo->mChallengeVeteranRecords[aChallengeIndex] > 0;
}
...
int LawnApp::GetNumTrophiesVeteran(ChallengePage thePage)
{
	int aNumTrophies = 0;
	...
	{
		const ChallengeDefinition& aDef = GetChallengeDefinition(i);
		if (aDef.mPage == thePage && HasBeatenChallengeVeteran(aDef.mChallengeMode))
		{
			aNumTrophies++;
		}
	}
...
{
	return  82 + 24 + 12 - GetNumTrophies(CHALLENGE_PAGE_SURVIVAL) - GetNumTrophies(CHALLENGE_PAGE_CHALLENGE) - GetNumTrophies(CHALLENGE_PAGE_PUZZLE);
}

int LawnApp::TrophiesNeedForDiamondSunflower()
{
	return  82 + 24 + 12 - GetNumTrophiesVeteran(CHALLENGE_PAGE_SURVIVAL) - GetNumTrophiesVeteran(CHALLENGE_PAGE_CHALLENGE) - GetNumTrophiesVeteran(CHALLENGE_PAGE_PUZZLE);
}
...
bool LawnApp::EarnedDiamondTrophy()
{
	return EarnedGoldTrophy() && TrophiesNeedForDiamondSunflower() <= 0;
}
...
SexyString LawnApp::GetMoneyString(int theAmount)
{
	...
	if (aValue >= 1000000)
	{
		return StrFormat(_S("$%.1fM"), aValue / 1000000.0);
	}
	else if (aValue >= 1000)
	{
		return StrFormat(_S("$%.1fk"), aValue / 1000.0);
	}
	else
	{
