...
void MessageWidget::SetLabel(const SexyString& theNewLabel, MessageStyle theMessageStyle)
{
	...
	else
	{
		...
		{	
			...
			{
				mReanimType = REANIM_TEXT_SLIDE_ON;
			}
			else 
			{
				ReanimationType rngTransition[] = { REANIM_TEXT_FADE_ON , REANIM_TEXT_SLIDE_DOWN, REANIM_TEXT_SLIDE_ON };
				mReanimType = rngTransition[Rand(3)];
			}
			mDuration = 750;
			break;
		}
...
void MessageWidget::Update()
{
	...
	for (int aPos = 0; aPos < aLabelLen; aPos++)
	{
		...
		// 设置动画速率
		int aTextSpeed = mReanimType >= ReanimationType::REANIM_TEXT_FADE_ON && mReanimType <= ReanimationType::REANIM_TEXT_SLIDE_ON ? 100 : 1;
		if (mDuration > mSlideOffTime)
		{
			if (mReanimType >= ReanimationType::REANIM_TEXT_FADE_ON && mReanimType <= ReanimationType::REANIM_TEXT_SLIDE_ON)
			{
...
void MessageWidget::DrawReanimatedText(Graphics* g, Font* theFont, const Color& theColor, float thePosY)
{
	...
	for (int aPos = 0; aPos < aLabelLen; aPos++)
	{
		...
		if (mReanimType == ReanimationType::REANIM_TEXT_FADE_ON && mReanimType <= ReanimationType::REANIM_TEXT_SLIDE_ON && mDisplayTime - mDuration < mSlideOffTime)
		{
...
void MessageWidget::Draw(Graphics* g)
{
	...
	case MessageStyle::MESSAGE_STYLE_HOUSE_NAME:
		aPosY = 550 + BOARD_OFFSET_Y;
		aColor = Color(255, 255, 255, 255);
		aFadeOut = true;
		break;

	case MessageStyle::MESSAGE_STYLE_HUGE_WAVE:
		aPosY = 330 + BOARD_OFFSET_Y;
	...
	else
	{
		...
		if (mMessageStyle == MessageStyle::MESSAGE_STYLE_HOUSE_NAME)
		{
			...
			{
				int aFlags = (mApp->mBoard->mIsVeteran ? (int)round(mApp->mBoard->GetNumWavesPerSurvivalStage() * mApp->mBoard->GetVeteranMultiplier()) : mApp->mBoard->GetNumWavesPerSurvivalStage()) * mApp->mBoard->mChallenge->mSurvivalStage / mApp->mBoard->GetNumWavesPerFlag();
