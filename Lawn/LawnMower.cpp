...
void LawnMower::UpdatePool()
{
    ...
    if (mPosX > 26.0f && mPosX < 660.0f && mBoard->mPlantRow[mRow] == PlantRowType::PLANTROW_POOL)
    {
        isPoolRange = true;
    }
...
void LawnMower::MowZombie(Zombie* theZombie)
{
    ...
    if (mMowerType == LawnMowerType::LAWNMOWER_POOL)
    {
        ...
        if (mMowerHeight == MowerHeight::MOWER_HEIGHT_IN_POOL && mBoard->mPlantRow[mRow] == PlantRowType::PLANTROW_POOL)
        {
            Reanimation* aMowerReanim = mApp->ReanimationGet(mReanimID);
...
void LawnMower::Update()
{
    ...
    while (mBoard->IterateZombies(aZombie))
    {
        ...
        if (aZombie->mZombieType != ZombieType::ZOMBIE_BOSS && 
            ...
            (aZombie->EffectedByDamage(127U) || aZombie->mZombieType == ZOMBIE_WRAITH))
        {
    ...
    if (mBoard->StageHasHighGrounds())
    {
        int aGridX = mBoard->PixelToGridXKeepOnBoard(mPosX + 30 + 42.5f, mPosY);
        if (mBoard->mGridSquareType[aGridX][mRow] == GridSquareType::GRIDSQUARE_HIGH_GROUND)
        {
            Die();
            mApp->AddTodParticle(mPosX, mPosY + 30.0f, mRenderOrder + 1, ParticleEffect::PARTICLE_CATAPULT_EXPLOSION);
