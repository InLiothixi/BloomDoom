...
void SyncBoard(SaveGameContext& theContext, Board* theBoard)
{
	...
	SyncDataArray(theContext, theBoard->mExtraPlants);
	SyncDataArray(theContext, theBoard->mPlayerController);
	SyncDataArray(theContext, theBoard->mZombies);													//0x482190
	SyncDataArray(theContext, theBoard->mBushesCheap);
	...
	theContext.SyncBytes(theBoard->mSeedBankP2, sizeof(SeedBank));
	...
	theContext.SyncBytes(&theBoard->mIsVeteran, sizeof(bool));
	theContext.SyncBytes(&theBoard->mMaxPlayers, sizeof(int));
...
{
	Plant* aExtraPlant = nullptr;
	while (theBoard->mExtraPlants.IterateNext(aExtraPlant))
	{
		aExtraPlant->mApp = theBoard->mApp;
		aExtraPlant->mBoard = theBoard;
	}

	PlayerController* aPlayer = nullptr;
	while (theBoard->mPlayerController.IterateNext(aPlayer))
	{
		aPlayer->mApp = theBoard->mApp;
		aPlayer->mBoard = theBoard;
	}
	...
	while (theBoard->mBushesCheap.IterateNext(aBushCheap))
	{
		aBushCheap->mApp = theBoard->mApp;
		aBushCheap->mBoard = theBoard;
	}
	...
	theBoard->mSeedBankP2->mApp = theBoard->mApp;
	theBoard->mSeedBankP2->mBoard = theBoard;

	for (int i = 0; i < SEEDBANK_MAX; i++)
	{
		theBoard->mSeedBankP2->mSeedPackets[i].mApp = theBoard->mApp;
		theBoard->mSeedBankP2->mSeedPackets[i].mBoard = theBoard;
	}
