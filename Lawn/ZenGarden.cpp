...
void ZenGarden::ZenGardenInitLevel(bool theJustSwitchingGardens)
{
    ...
    mApp->mBoard->HideSpeedButton();

    if (mGardenType == GardenType::GARDEN_MUSHROOM)
        mApp->UpdateDiscordRPC(TodStringTranslate(_S("[MUSHROOM]")), TodStringTranslate(_S("[MUSHROOM_RICH_PRESENCE]")));
    else if (mGardenType == GardenType::GARDEN_AQUARIUM)
        mApp->UpdateDiscordRPC(TodStringTranslate(_S("[AQUARIUM]")), TodStringTranslate(_S("[AQUARIUM_RICH_PRESENCE]")));
    else
        mApp->UpdateDiscordRPC(TodStringTranslate(_S("[ZEN_GARDEN]")), TodStringTranslate(_S("[ZEN_GARDEN_RICH_PRESENCE]")));
}
...
void ZenGarden::StinkyUpdate(GridItem* theStinky)
{
    ...
    mApp->mPlayerInfo->mLastTimeStinkyAwake = _time32(nullptr);
}
...
void ZenGarden::ZenGardenUpdate()
{
    ...
    int currentTime = _time32(nullptr);

    int timeElapsed = currentTime - mApp->mPlayerInfo->mLastTimeVisitedZen;
    if (!mCheckedIdle) {
        if (timeElapsed < 10) {
            mCheckedIdle = true;
        }
        else if (mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_STINKY_THE_SNAIL]) {
            mCheckedIdle = true;
            int stinkyTime = currentTime - mApp->mPlayerInfo->mLastTimeStinkyAwake;
            if (stinkyTime > 0 && mApp->mPlayerInfo->mLastTimeVisitedZen - mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_STINKY_THE_SNAIL] <= 180) {
                ...
                while (mBoard->IteratePlants(aPlant)) {
                    if (aPlant->mPottedPlantIndex != -1) {
                        ...
                        bool chocolated2 = mApp->mPlayerInfo->mLastTimeVisitedZen - aPottedPlant->mLastChocolateTime < 300;
                        int minutes2 = chocolated2 ? 0 : (mApp->mPlayerInfo->mLastTimeVisitedZen - aPottedPlant->mLastNeedFulfilledTime) / 60;
                        int aCounterMax2 = TodAnimateCurve(5, 30, minutes2, 3000, 15000, TodCurves::CURVE_LINEAR);
                        aPlant->mLaunchCounter = RandRangeInt(1800, aCounterMax2);

                        for (int i = 0; i < stinkyTime; ++i) {
                            if (mApp->mPlayerInfo->mLastTimeVisitedZen + i - aPottedPlant->mLastFertilizedTime <= 3600) continue;
                            //if (mApp->mPlayerInfo->mLastTimeVisitedZen + i - mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_STINKY_THE_SNAIL] > 180)  continue;

                            bool chocolated = mApp->mPlayerInfo->mLastTimeVisitedZen + i - aPottedPlant->mLastChocolateTime < 300;
                            int minutes = chocolated ? 0 : (mApp->mPlayerInfo->mLastTimeVisitedZen + i - aPottedPlant->mLastNeedFulfilledTime) / 60;
                            int aCounterMax = TodAnimateCurve(5, 30, minutes, 3000, 15000, TodCurves::CURVE_LINEAR);

                            aPlant->mLaunchCounter -= 100;
                            if (chocolated) {
                                aPlant->mLaunchCounter -= 100;
                            }

                            if (aPlant->mLaunchCounter <= 0) {
                                aPlant->mLaunchCounter = RandRangeInt(1800, aCounterMax);

                                int aCoinHit = Rand(1000);
                                aCoinHit += TodAnimateCurve(5, 30, minutes, 0, 80, TodCurves::CURVE_LINEAR);

                                if (aCoinHit < 100) {
                                    moneyCollected += 5;
                                    mApp->mPlayerInfo->AddCoins(5);
                                }
                                else {
                                    moneyCollected += 1;
                                    mApp->mPlayerInfo->AddCoins(1);
                                }
                            }
                        }
                    }
                ...
                if (moneyCollected > 0) {
                    ...
                    string moneyTxt = to_string(moneyCollected);
                    int n = moneyTxt.length() - 3;
                    while (n > 0) {
                        moneyTxt.insert(n, ",");
                        n -= 3;
                    }
                    string resultsTxt = "While you were away, Stinky collected " + moneyTxt + " coins";
                    if (currentTime - mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_STINKY_THE_SNAIL] > 180) {
                        resultsTxt += " before\nfalling asleep";
                    }
                    resultsTxt += "!";
                    ...
                    mBoard->DisplayAdvice(resultsTxt, MESSAGE_STYLE_HINT_TALL_UNLOCKMESSAGE, ADVICE_NONE);
                }
            }
        }
    }
    ...
    mApp->mPlayerInfo->mLastTimeVisitedZen = _time32(nullptr);
...
void ZenGarden::GotoNextGarden()
{
    ...
    if (aGoToTree)
    {
        ...
        mApp->UpdateDiscordRPC(TodStringTranslate(_S("[TREE_OF_WISDOM]")), TodStringTranslate(_S("[TREE_OF_WISDOM_RICH_PRESENCE]")));
        return;
    }
