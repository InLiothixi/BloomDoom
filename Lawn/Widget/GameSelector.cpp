...
GameSelector::GameSelector(LawnApp* theApp)
{
	...
	mApp->UpdateDiscordRPC(TodStringTranslate(_S("[MAIN_MENU]")), TodStringTranslate(_S("[MAIN_MENU_RICH_PRESENCE]")));
	...
	/*if (mApp->mPlayerInfo)
	{
		...
		mApp->mPlayerInfo->mName = "Ultra Wide Expansion";
		for (int i = 1; i < NUM_GAME_MODES; i++)
			if (i != (int)GameMode::GAMEMODE_TREE_OF_WISDOM && i != (int)GameMode::GAMEMODE_SCARY_POTTER_ENDLESS &&
				i != (int)GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_ENDLESS && i != (int)GameMode::GAMEMODE_SURVIVAL_ENDLESS_STAGE_6)
...
void GameSelector::SyncButtons()
{
	...
	if (mShowStartButton)
	{
		...
		aSelectorReanim->AssignRenderGroupToPrefix("SelectorScreen_StartAdventure_", RENDER_GROUP_NORMAL);
...
void GameSelector::Draw(Graphics* g)
{
	...
	if (mHasTrophy)
	{
		if (mApp->EarnedDiamondTrophy())
		{
			TodDrawImageScaledF(g, Sexy::IMAGE_DIAMOND_SUNFLOWER_TROPHY, aTransformLeft.mTransX + 10.0f + BOARD_ADDITIONAL_WIDTH + 370 - 6, aTransformLeft.mTransY + 390.0f + BOARD_OFFSET_Y + 60, 1, 1);
		}
		else if (mApp->EarnedGoldTrophy())
		{
...
void GameSelector::DrawOverlay(Graphics* g)
{
	...
	if (!mApp->IsIceDemo() && !mShowStartButton)
	{
		...
		if (!mShowStartButton)
		{
			...
			TodDrawImageCelF(g, Sexy::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransAreaX + 486.0f + 111, aTransAreaY + 125.0f - 77.5f + 80, aStage, 0);  // 绘制大关数
			if (aSub < 10)
			{
				TodDrawImageCelF(g, Sexy::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransSubX + 508.5f + 111, aTransSubY + 128.0f - 77.5f + 80, aSub, 0);
			}
			else if (aSub == 10)
			{
				TodDrawImageCelF(g, Sexy::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransSubX + 504.0f + 111, aTransSubY + 128.0f - 77.5f + 80, 1, 0);
				TodDrawImageCelF(g, Sexy::IMAGE_SELECTORSCREEN_LEVELNUMBERS, aTransSubX + 513.0f + 111, aTransSubY + 129.0f - 77.5f + 80, 0, 0);
			}
	...
	if (mApp->mBetaValidate)
	{
		...
		if (gIsPartnerBuild)
			g->DrawString(_S("PRESS/PARTNER PREVIEW BUILD: DO NOT DISTRIBUTE"), 27, 594 + BOARD_OFFSET_Y * 2);
		else
			g->DrawString(_S("BETA BUILD: DO NOT DISTRIBUTE"), 27, 594 + BOARD_OFFSET_Y * 2);
	}
...
void GameSelector::Update()
{
	...
	if (mStartingGame)
	{
		...
		if (mStartingGameCounter > 450)
		{
			...
			if (mApp->IsIceDemo())
			{
				mApp->PreNewGame(GameMode::GAMEMODE_CHALLENGE_ICE_LEVEL, false);
				return;
			}
	...
	case SelectorAnimState::SELECTOR_OPEN:
		...
		if (aSelectorReanim->mLoopCount > 0)
		{
			...
			{
				aSelectorReanim->PlayReanim("anim_sign", ReanimLoopType::REANIM_LOOP, 0, 30.0f);
...
void GameSelector::KeyChar(char theChar)
{
	...
	if ((gIsPartnerBuild || mApp->mDebugKeysEnabled) && theChar == 'p' && mApp->mPlayerInfo)
	{
		TodTraceAndLog(_S("Selector cheat key '%c'"), theChar);
		mApp->mPlayerInfo->mFinishedAdventure = 1;
		mApp->mPlayerInfo->mCoins = 150000;
		mApp->mPlayerInfo->mHasUsedCheatKeys = true;
		mApp->mPlayerInfo->mHasUnlockedMinigames = true;
		mApp->mPlayerInfo->mHasUnlockedPuzzleMode = true;
		mApp->mPlayerInfo->mHasUnlockedSurvivalMode = true;
		memset(mApp->mPlayerInfo->mChallengeRecords, 20, sizeof(mApp->mPlayerInfo->mChallengeRecords));
		memset(mApp->mPlayerInfo->mChallengeVeteranRecords, 20, sizeof(mApp->mPlayerInfo->mChallengeVeteranRecords));
		mApp->mPlayerInfo->mChallengeVeteranRecords[GAMEMODE_CHALLENGE_ZOMBOTANY - 1] = 0;
		SyncProfile(true);
		mApp->EraseFile(GetSavedGameName(GameMode::GAMEMODE_ADVENTURE, mApp->mPlayerInfo->mId));
	}

	if ((gIsPartnerBuild || mApp->mDebugKeysEnabled) && theChar == 'i' && mApp->mPlayerInfo)
	{
		TodTraceAndLog(_S("Selector cheat key '%c'"), theChar);
		mApp->mPlayerInfo->mFinishedAdventure = 1;
		mApp->mPlayerInfo->mCoins = 50000;
		mApp->mPlayerInfo->mHasUsedCheatKeys = true;
		mApp->mPlayerInfo->mHasUnlockedMinigames = true;
		mApp->mPlayerInfo->mHasUnlockedPuzzleMode = true;
		mApp->mPlayerInfo->mHasUnlockedSurvivalMode = true;
		memset(mApp->mPlayerInfo->mChallengeRecords, 20, sizeof(mApp->mPlayerInfo->mChallengeRecords));
		memset(mApp->mPlayerInfo->mChallengeVeteranRecords, 0, sizeof(mApp->mPlayerInfo->mChallengeVeteranRecords));
		mApp->mPlayerInfo->mChallengeRecords[GAMEMODE_CHALLENGE_ZOMBOTANY - 1] = 0;
		SyncProfile(true);
		mApp->EraseFile(GetSavedGameName(GameMode::GAMEMODE_ADVENTURE, mApp->mPlayerInfo->mId));
	}
	...
	if ((gIsPartnerBuild || mApp->mDebugKeysEnabled) && theChar == 'o' && mApp->mPlayerInfo)
	{
		TodTraceAndLog(_S("Selector cheat key '%c'"), theChar);
		mApp->mPlayerInfo->mFinishedAdventure = 2;
		mApp->mPlayerInfo->mCoins = 150000;
	...
	if ((gIsPartnerBuild || mApp->mDebugKeysEnabled) && theChar == 't' && mApp->mPlayerInfo)
	{
		TodTraceAndLog(_S("Selector cheat key '%c'"), theChar);
		mApp->mPlayerInfo->mFinishedAdventure = 1;
		mApp->mPlayerInfo->mCoins = 10000;
		mApp->mPlayerInfo->mHasUsedCheatKeys = true;
		mApp->mPlayerInfo->mHasUnlockedMinigames = true;
		mApp->mPlayerInfo->mHasUnlockedPuzzleMode = true;
		mApp->mPlayerInfo->mHasUnlockedSurvivalMode = true;
		memset(mApp->mPlayerInfo->mChallengeRecords, 0, sizeof(mApp->mPlayerInfo->mChallengeRecords));
		memset(mApp->mPlayerInfo->mChallengeVeteranRecords, 0, sizeof(mApp->mPlayerInfo->mChallengeVeteranRecords));
		SyncProfile(true);
		mApp->EraseFile(GetSavedGameName(GameMode::GAMEMODE_ADVENTURE, mApp->mPlayerInfo->mId));
	}

	if ((gIsPartnerBuild || mApp->mDebugKeysEnabled) && theChar == 'y' && mApp->mPlayerInfo)
	{
		TodTraceAndLog(_S("Selector cheat key '%c'"), theChar);
		mApp->mPlayerInfo->mFinishedAdventure = 1;
		mApp->mPlayerInfo->mCoins = 10000;
...
void GameSelector::ButtonDepress(int theId)
{
	...
	case GameSelector::GameSelector_ZenGarden:
		mApp->KillGameSelector();
		mApp->PreNewGame(GameMode::GAMEMODE_CHALLENGE_ZEN_GARDEN, false);
		mApp->mZenGarden->mCheckedIdle = false;
		if (ShouldDoZenTuturialBeforeAdventure())
			mApp->mZenGarden->SetupForZenTutorial();
		break;
	}
