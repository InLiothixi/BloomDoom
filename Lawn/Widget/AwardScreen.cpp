...
AwardScreen::AwardScreen(LawnApp* theApp, AwardType theAwardType)
{
    ...
    mMenuButton->Resize(677 + BOARD_ADDITIONAL_WIDTH, 16 + BOARD_OFFSET_Y, 111, 26);
    ...
    if (IsPaperNote())
    {
        ...
        {
            mApp->mMusic->MakeSureMusicIsPlaying(MUSIC_TUNE_HELP);
        }
        else 
        {
...
void AwardScreen::DrawAwardSeed(Graphics* g)
{
    ...
    DrawSeedPacket(g, 350 + BOARD_ADDITIONAL_WIDTH, 129 + BOARD_OFFSET_Y, aSeedType, SEED_NONE, 0, 255, true, false, true);
    g->SetScale(1, 1, 0, 0);
}
...
void AwardScreen::Draw(Graphics* g)
{
    ...
    if (mAwardType == AWARD_CREDITS_ZOMBIENOTE)
    {
        ...
        g->DrawImage(Sexy::IMAGE_BACKGROUND6BOSS, -900 + BOARD_ADDITIONAL_WIDTH, -400 + BOARD_OFFSET_Y, 2800, 1200);
    ...
    else
    {
        if (!mApp->IsAdventureMode())
        {
            ...
            {
                DrawBottom(g, _S("[BEAT_GAME_MESSAGE3]"), _S("[DIAMOND_SUNFLOWER_TROPHY]"), _S("[BEAT_GAME_MESSAGE4]"));
                TodDrawImageCenterScaledF(g, Sexy::IMAGE_DIAMOND_SUNFLOWER_TROPHY, 315 + BOARD_ADDITIONAL_WIDTH - 6, 60 + BOARD_OFFSET_Y, 0.7f, 0.7f);
            }
            else if (mApp->EarnedGoldTrophy())
            {
                DrawBottom(g, _S("[BEAT_GAME_MESSAGE1]"), _S("[GOLD_SUNFLOWER_TROPHY]"), _S("[BEAT_GAME_MESSAGE2]"));
                TodDrawImageCelCenterScaledF(g, Sexy::IMAGE_SUNFLOWER_TROPHY, 315 + BOARD_ADDITIONAL_WIDTH, 60 + BOARD_OFFSET_Y, 1, 0.7f, 0.7f);
            }
            else
            {
                const char* aMsgChar;
