...
void DrawStoneButton(Graphics* g, int x, int y, int theWidth, int theHeight, bool isDown, bool isHighLighted, const SexyString& theLabel)
{
	...
	aFontX += (aLeftImage->mWidth * 2 + aMiddleImage->mWidth * ((theWidth - aLeftImage->mWidth - aRightImage->mWidth) / aMiddleImage->mWidth) - Sexy::FONT_DWARVENTODCRAFT18GREENINSET->StringWidth(theLabel)) / 2 + 1;
