...
static StoreItem gStoreItemSpots[NUM_STORE_PAGES][MAX_PAGE_SPOTS] =
{
    ...
    { STORE_ITEM_PLANT_SPIKEROCK,   STORE_ITEM_PLANT_GOLD_MAGNET,   STORE_ITEM_PLANT_WINTERMELON,   STORE_ITEM_PLANT_COBCANNON,
      STORE_ITEM_PLANT_IMITATER,    STORE_ITEM_PLANT_EXPENSIVE_NUT, STORE_ITEM_FIRSTAID,             STORE_ITEM_INVALID },
    ...
    { STORE_ITEM_INVALID,   STORE_ITEM_INVALID,     STORE_ITEM_INVALID,        STORE_ITEM_INVALID,
      STORE_ITEM_INVALID,    STORE_ITEM_THREE_MALLETS,           STORE_ITEM_SIX_MALLETS,                 STORE_ITEM_NINE_MALLETS }
};
...
StoreScreen::StoreScreen(LawnApp* theApp) : Dialog(nullptr, nullptr, DIALOG_STORE, true, _S("Store"), _S(""), _S(""), BUTTONS_NONE)
{
    ...
    mBackButton->Resize(366 + BOARD_ADDITIONAL_WIDTH - 75.0f / 2.0f, 512 + BOARD_OFFSET_Y + 12, aMenuImage->mWidth, aMenuImage->mHeight);
    ...
    mPrevButton->Resize(185 + BOARD_ADDITIONAL_WIDTH, 388 + BOARD_OFFSET_Y, aPrevImage->mWidth, aPrevImage->mHeight);
    ...
    mNextButton->Resize(586 + BOARD_ADDITIONAL_WIDTH, 388 + BOARD_OFFSET_Y, aNextImage->mWidth, aNextImage->mHeight);
    ...
    mApp->UpdateDiscordRPC(TodStringTranslate(_S("[SHOP]")), TodStringTranslate(_S("[SHOP_RICH_PRESENCE]")));
}
...
bool StoreScreen::IsItemUnavailable(StoreItem theStoreItem)
{
    ...
    */
    ...
    if (theStoreItem == STORE_ITEM_PLANT_WINTERMELON || theStoreItem == STORE_ITEM_PLANT_COBCANNON ||
        theStoreItem == STORE_ITEM_PLANT_IMITATER || theStoreItem == STORE_ITEM_PLANT_EXPENSIVE_NUT || theStoreItem == STORE_ITEM_FIRSTAID)
    {
        return !mApp->HasFinishedAdventure();
    }
    return false;
...
//0x48AC50
void StoreScreen::DrawItemIcon(Graphics* g, int theItemPosition, StoreItem theItemType, bool theIsForHighlight, float offsetX, float offsetY)
{
    ...
    else if (theItemType >= STORE_ITEM_THREE_MALLETS && theItemType <= STORE_ITEM_NINE_MALLETS)
    {
        g->DrawImage(Sexy::IMAGE_HAMMER_ICON, aPosX - 6, aPosY + 3);

        int count = 3 + 3 * ((int)theItemType - (int)STORE_ITEM_THREE_MALLETS);
        TodDrawString(g, _S("x" + to_string(count)), aPosX + 56, aPosY + 62, Sexy::FONT_HOUSEOFTERROR16, Color::White, DS_ALIGN_RIGHT);
    }
    ...
    {
        if (theItemType == STORE_ITEM_PLANT_EXPENSIVE_NUT)
        {
            DrawSeedPacket(g, aPosX, aPosY, SEED_DUMMY, SEED_NONE, 0, 255, false, false);
        }
        else
        {
            DrawSeedPacket(g, aPosX, aPosY, (SeedType)(theItemType + (int)SEED_GATLINGPEA), SEED_NONE, 0, 255, false, false);
        }
    }
...
//0x48B170
void StoreScreen::DrawItem(Graphics* g, int theItemPosition, StoreItem theItemType, float offsetX, float offsetY)
{
    if (IsItemUnavailable(theItemType))
        return;

    DrawItemIcon(g, theItemPosition, theItemType, false, offsetX, offsetY);
    ...
    {
        if (theItemType >= 0 && theItemType <= 9)
        {
            g->DrawImage(Sexy::IMAGE_SEEDPACKETFLASH, aPosX, aPosY);
        }
        else
        {
            DrawItemIcon(g, theItemPosition, theItemType, true, offsetX, offsetY);
        }
    }
}
...
void StoreScreen::Draw(Graphics* g)
{
    ...
    int aStoreSignPosY = TodAnimateCurve(50, 110, mStoreTime, -150, -63, CURVE_EASE_IN_OUT);
    if (mApp->IsNight())
    {
        g->DrawImage(Sexy::IMAGE_STORE_BACKGROUNDNIGHT, -BOARD_ADDITIONAL_WIDTH, -BOARD_OFFSET_Y);
    }
    else
    {
        g->DrawImage(Sexy::IMAGE_STORE_BACKGROUND, -BOARD_ADDITIONAL_WIDTH, -BOARD_OFFSET_Y);
    }

    float carOffsetX = -75.0f, carOffsetY = -55.0f;
    
    if (mApp->IsNight())
        g->DrawImage(Sexy::IMAGE_STORE_CAR_NIGHT, mShakeX + 196 + carOffsetX, mShakeY + 138 + carOffsetY);
    else
        g->DrawImage(Sexy::IMAGE_STORE_CAR, mShakeX + 196 + carOffsetX, mShakeY + 138 + carOffsetY);

    if (!mHatchTimer && mHatchOpen)
    {
        g->DrawImage(Sexy::IMAGE_STORE_HATCHBACKOPEN, mShakeX + 299 + carOffsetX, mShakeY + carOffsetY);
    }
    else  
    {
        g->DrawImage(Sexy::IMAGE_STORE_CARCLOSED, mShakeX + 196, mShakeY + 138 + carOffsetY);
        if (mApp->IsNight())
            g->DrawImage(Sexy::IMAGE_STORE_CARCLOSED_NIGHT, mShakeX + 256, mShakeY + 163 + carOffsetY);
    ...
    gCrazyDave.mTransX -= 42.0f - carOffsetX / 2.0f + 25.0f;
    gCrazyDave.mTransY += 68.0f;
    mApp->DrawCrazyDave(&gCrazyDave);

    if (!mHatchTimer && mHatchOpen)
    {
        int offsetX = BOARD_ADDITIONAL_WIDTH + carOffsetX / 2.0f - 12.5f, offsetY = BOARD_OFFSET_Y + carOffsetY / 2.0f;
        for (int i = 0; i < MAX_PAGE_SPOTS; i++)
        {
            StoreItem aStoreItem = GetStoreItemType(i);
            if (aStoreItem != STORE_ITEM_INVALID)
            {
                float calX = i % 4 / 4.0f * 77.0f;
                if (i >= 4)
                    calX += 6.25f;
                DrawItem(g, i, aStoreItem, offsetX + calX, offsetY);
            }
        }
    }

    g->mTransX += BOARD_ADDITIONAL_WIDTH * 2;
    g->mTransY += BOARD_OFFSET_Y * 2;
    ...
    if (!mPrevButton->mDisabled)
    {
        ...
        TodDrawString(g, aPageString, STORESCREEN_PAGESTRING_X + BOARD_ADDITIONAL_WIDTH - 75.0f / 2, STORESCREEN_COINBANK_Y, Sexy::FONT_BRIANNETOD12, Color(80, 80, 80), DS_ALIGN_CENTER);
    }
}
...
void StoreScreen::UpdateMouse()
{
    ...
    for (int aItemPos = 0; aItemPos < MAX_PAGE_SPOTS; aItemPos++)
    {
        ...
        if (aItemType != STORE_ITEM_INVALID && !IsItemUnavailable(aItemType))
        {
            ...
            aItemX += BOARD_ADDITIONAL_WIDTH - 37.5f;
            aItemY += 27.5f;

            float calX = aItemPos % 4 / 4.0f * 77.0f;
            if (aItemPos >= 4)
                calX += 6.25f;

            if (Rect(aItemX, aItemY, 50, 87).Contains(aMouseX - calX + 12.5f, aMouseY))
            {
                ...
                case STORE_ITEM_PLANT_EXPENSIVE_NUT:    aMessageIndex = 2035;                           break;
    ...
    else if (theId == StoreScreen::StoreScreen_Prev || theId == StoreScreen::StoreScreen_Next)
    {
        ...
        do
        {
            if (theId == StoreScreen::StoreScreen_Prev)
            {
                ...
                {
                    mPage = (StorePages)((int)NUM_STORE_PAGES - 1);
                }
            }
            else
            {
...
int StoreScreen::GetItemCost(StoreItem theStoreItem)
{
    ...
    case STORE_ITEM_PLANT_EXPENSIVE_NUT:                return 100000;
    ...
    case STORE_ITEM_WHEEL_BARROW:                       return 200;
    ...
    case STORE_ITEM_THREE_MALLETS:                      return 300;
    case STORE_ITEM_SIX_MALLETS:                        return 500;
    case STORE_ITEM_NINE_MALLETS:                       return 700;
...
void StoreScreen::AdvanceCrazyDaveDialog()
{
    ...
    else if (aMessage == 603)
    {
        ...
        aCoin->CoinInitialize(80 + BOARD_ADDITIONAL_WIDTH, 520 + BOARD_OFFSET_Y, COIN_DIAMOND, COIN_MOTION_FROM_PRESENT);
        aCoin->mVelX = 0;
        aCoin->mVelY = -5;
    }
...
{
    x -= BOARD_ADDITIONAL_WIDTH - 37.5f;
    y -= BOARD_OFFSET_Y - 27.5f;
    ...
    for (int aItemPos = 0; aItemPos < MAX_PAGE_SPOTS; aItemPos++)
    {
        ...
        if (aItemPos >= 4)
            calX += 6.25f;
        if (Rect(aItemX, aItemY, 50, 87).Contains(x - calX + 12.5f, y))
        {
