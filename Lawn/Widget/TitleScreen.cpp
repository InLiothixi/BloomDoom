...
{
	g->DrawImageF(IMAGE_PLANTSHADOW, 1000.0f + 480, 0.0f);
}
...
void TitleScreen::Update()
{
	...
	if (mNeedToInit)
	{
		...
		mStartButton->Resize(mWidth / 2 - IMAGE_LOADBAR_DIRT->mWidth / 2, 650 + BOARD_OFFSET_Y + 10, mTotalBarWidth, 50);
	...
	{
		aButtonY = TodAnimateCurve(60, 10, mTitleStateCounter, 650 + BOARD_OFFSET_Y + 10, 534 + BOARD_OFFSET_Y, TodCurves::CURVE_EASE_IN);
	}
	else
	{
		aButtonY = TodAnimateCurve(10, 0, mTitleStateCounter, 534 + BOARD_OFFSET_Y + 10, 529 + BOARD_OFFSET_Y, TodCurves::CURVE_BOUNCE);
	}
	...
	for (int i = 0; i < LENGTH(aTriggerPoint); i++)
	{
		if (aPrevWidth < aTriggerPoint[i] && mCurBarWidth >= aTriggerPoint[i])
		{
			...
			float aPosX = aTriggerPoint[i] + 225.0f + BOARD_ADDITIONAL_WIDTH;
