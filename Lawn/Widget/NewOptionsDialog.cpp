    ...
    Dialog(nullptr, nullptr, Dialogs::DIALOG_NEWOPTIONS, true, _S("Options"), _S(""), _S(""), Dialog::BUTTONS_NONE)
{
    ...
    mLanguageButton = MakeButton(NewOptionsDialog::NewOptionsDialog_MainMenu, this, "Language");
    mHotkeysButton = MakeButton(NewOptionsDialog::NewOptionsDialog_MainMenu, this, "Edit Controls");
    ...
    mHQMusicCheckbox = MakeNewCheckbox(NewOptionsDialog::NewOptionsDialog_HQMusic, this, theApp->mHQMusic);
    mVeteranModeCheckbox = MakeNewCheckbox(NewOptionsDialog::NewOptionsDialog_VeteranMode, this, theApp->mVeteranMode);
    mAutoPauseCheckbox = MakeNewCheckbox(NewOptionsDialog::NewOptionsDialog_AutoPause, this, theApp->mAutoPause);
    ...
    if (mFromGameSelector)
    {
        ...
        mHotkeysButton->SetVisible(false);
        mHQMusicCheckbox->SetVisible(false);
        mVeteranModeCheckbox->SetVisible(false);
        mAutoPauseCheckbox->SetVisible(false);
    }
...
void NewOptionsDialog::AddedToManager(Sexy::WidgetManager* theWidgetManager)
{
    ...
    //AddWidget(mHotkeysButton);
    AddWidget(mMusicVolumeSlider);
    AddWidget(mSfxVolumeSlider);
    AddWidget(mHardwareAccelerationCheckbox);
    AddWidget(mHQMusicCheckbox);
    AddWidget(mVeteranModeCheckbox);
    AddWidget(mAutoPauseCheckbox);
...
void NewOptionsDialog::RemovedFromManager(Sexy::WidgetManager* theWidgetManager)
{
    ...
    RemoveWidget(mHQMusicCheckbox);
    RemoveWidget(mVeteranModeCheckbox);
    RemoveWidget(mAutoPauseCheckbox);
    RemoveWidget(mBackToMainButton);
    //RemoveWidget(mLanguageButton);
    //RemoveWidget(mHotkeysButton);
...
void NewOptionsDialog::Resize(int theX, int theY, int theWidth, int theHeight)
{
    ...
    mMusicVolumeSlider->Resize(199 - 36, 116, 135, 40);
    mSfxVolumeSlider->Resize(199 - 36, 143, 135, 40);
    mHardwareAccelerationCheckbox->Resize(283, 175, 46, 45);
    mFullscreenCheckbox->Resize(284, 206, 46, 45);
    mHQMusicCheckbox->Resize(283, 237, 46, 45);
    mVeteranModeCheckbox->Resize(283, 268, 46, 45);
    mAutoPauseCheckbox->Resize(283, 299, 46, 45);
    ...
    if (mFromGameSelector) {
        ...
        mHQMusicCheckbox->mY += 25;
        mVeteranModeCheckbox->mY += 30;
        mAutoPauseCheckbox->mY += 35;

        mBackToMainButton->Resize(mRestartButton->mX, mRestartButton->mY + 43, 209, 46);
        //mLanguageButton->Resize(mBackToMainButton->mX + 112.5f + 5, mRestartButton->mY + 43, 150, 46);
       // mHotkeysButton->Resize(mRestartButton->mX, mRestartButton->mY + 43 - 41.5f - 5, 209, 46);
    }
    else {
        mBackToMainButton->Resize(mRestartButton->mX, mRestartButton->mY + 43, 209, 46);;
    }
...
void NewOptionsDialog::Draw(Sexy::Graphics* g)
{
    ...
    int aHQMusicOffset = 0;
    int aVeteranModeOffset = 0;
    int aAutoPauseOffset = 0;
    if (mFromGameSelector)
    {
        aMusicOffset = 5;
        aSfxOffset = 10;
    ...
    if (mFromGameSelector)
    {
        TodDrawString(g, _S("HQ Music"), 274, 261 + aHQMusicOffset, FONT_DWARVENTODCRAFT18, aTextColor, DrawStringJustification::DS_ALIGN_RIGHT);
        TodDrawString(g, _S("Veteran Mode"), 274, 293 + aVeteranModeOffset, FONT_DWARVENTODCRAFT18, aTextColor, DrawStringJustification::DS_ALIGN_RIGHT);
        TodDrawString(g, _S("Auto Pause"), 274, 325 + aAutoPauseOffset, FONT_DWARVENTODCRAFT18, aTextColor, DrawStringJustification::DS_ALIGN_RIGHT);
    }
}
...
void NewOptionsDialog::CheckboxChecked(int theId, bool checked)
{
    switch (theId)
    {
    case NewOptionsDialog::NewOptionsDialog_AutoPause:
    {
        mApp->mAutoPause = checked;
        break;
    }
    case NewOptionsDialog::NewOptionsDialog_HQMusic:
    {
        mApp->mHQMusic = checked;
        break;
    }
    case NewOptionsDialog::NewOptionsDialog_VeteranMode:
    {
        mApp->mVeteranMode = checked;
        break;
    }
