...
const Rect cZombieRect = Rect(22 + BOARD_ADDITIONAL_WIDTH, 86 + BOARD_OFFSET_Y, 510, 480);
const Rect cPlantRect = Rect(26 + BOARD_ADDITIONAL_WIDTH, 92 + BOARD_OFFSET_Y, 416, 465);
...
AlmanacDialog::AlmanacDialog(LawnApp* theApp) : LawnDialog(theApp, DIALOG_ALMANAC, true, _S("Almanac"), _S(""), _S(""), BUTTONS_NONE)
{
	...
	mTextSlider = new Slider(IMAGE_OPTIONS_SLIDERSLOT4, IMAGE_OPTIONS_SLIDERKNOB3, 0, this);
	mTextSlider->SetValue(max(0.0, min(mMaxScrollPosition, mScrollPosition)));
	mTextSlider->mHorizontal = false;
	mTextSlider->Resize(778 + BOARD_ADDITIONAL_WIDTH + 10, 152, 40, 475);
	mTextSlider->mThumbOffsetX -= 14;
	mTextSlider->mVisible = false;
	...
	mApp->UpdateDiscordRPC(TodStringTranslate(_S("[SUBURBAN_ALMANAC]")), TodStringTranslate(_S("[SUBURBAN_ALMANAC_RICH_PRESENCE]")));
}
...
void AlmanacDialog::RemovedFromManager(WidgetManager* theWidgetManager)
{
	...
	LawnDialog::RemoveWidget(mSlider);
	ClearPlantsAndZombies();
}
...
void AlmanacDialog::SetupPlant()
{
	...
	mMaxScrollPosition = 78 * max(0, floor(((int)SeedType::NUM_SEEDS_IN_CHOOSER - (int)SEED_COBCANNON) / 6));
	...
	else if (mSelectedSeed == SEED_FLOWERPOT || mSelectedSeed == SEED_WATERPOT)		aPosY -= 20;
	else if (mSelectedSeed == SEED_INSTANTCOFFEE)	aPosY += 20;
	...
	mPlant->mX = aPosX;
	if (mPlant->mSeedType == SEED_CHERRYHOVERBOMB)
		mPlant->mX += 25;
	mPlant->mY = aPosY;

	mApp->UpdateDiscordRPC("The Suburban Almanac (Plants)", "Reading");
}
...
void AlmanacDialog::SetupZombie()
{
	...
	mMaxScrollPosition = 80 * max(0, floor(((int)ZombieType::NUM_ZOMBIE_TYPES - (int)ZOMBIE_BOSS) / 5));
	mSlider->mVisible = true;
	...
	mApp->UpdateDiscordRPC("The Suburban Almanac (Zombies)", "Reading Entries");
}
...
void AlmanacDialog::SetPage(AlmanacPage thePage)
{
	...
	if (mOpenPage == AlmanacPage::ALMANAC_PAGE_INDEX)
	{
		...
		mApp->UpdateDiscordRPC("The Suburban Almanac", "Reading Entries");
	}
	else
	{
		mScrollPosition = mMaxScrollPosition = 0;
		//mSelectedSeed = (SeedType)0;
		//mSelectedZombie = (ZombieType)0;
...
void AlmanacDialog::KeyDown(Sexy::KeyCode keyCode) {
	
	switch (keyCode) {
		case KEYCODE_DOWN:
		{
			mScrollAmount += 1.666666f;
			break;
		}
		case KEYCODE_UP:
		{
...
void AlmanacDialog::DrawPlants(Graphics* g)
{
	...
	TodDrawString(g, _S("[SUBURBAN_ALMANAC_PLANTS]"), BOARD_WIDTH / 2, 48 + BOARD_OFFSET_Y, Sexy::FONT_HOUSEOFTERROR20, Color(213, 159, 43), DrawStringJustification::DS_ALIGN_CENTER);
	...
	for (int i = 0; i < NUM_ALMANAC_ZOMBIES; i++)
	{
		...
		if (aZombieType != ZombieType::ZOMBIE_INVALID)
		{
			...
			else
			{
				...
				if (aPosY + 2 + 72 > cZombieRect.mY + cZombieRect.mHeight)
				{
					aZombieGraphics.mClipRect.mY = aPosY;
					aZombieGraphics.mClipRect.mHeight = cZombieRect.mHeight - aPosY + 84 + BOARD_OFFSET_Y;
				}
				else if (aPosY + 2 < cZombieRect.mY)
				{
					aZombieGraphics.mClipRect.mY = cZombieRect.mY;
					aZombieGraphics.mClipRect.mHeight = aPosY + 72 - cZombieRect.mY;
				}
...
void AlmanacDialog::GetSeedPosition(SeedType theSeedType, int& x, int& y)
{
	...
	{
		if (theSeedType > SeedType::SEED_IMITATER)
			theSeedType = (SeedType)((int)theSeedType - 1);
...
SeedType AlmanacDialog::SeedHitTest(int x, int y)
{
	...
	if (mMouseVisible && mOpenPage == AlmanacPage::ALMANAC_PAGE_PLANTS)
	{
		for (SeedType aSeedType = SeedType::SEED_PEASHOOTER; aSeedType < NUM_ALMANAC_SEEDS; aSeedType = (SeedType)(aSeedType + 1))
		{
			if (y + BOARD_OFFSET_Y < cPlantRect.mY + cPlantRect.mHeight && y + BOARD_OFFSET_Y > cPlantRect.mY && aSeedType != SEED_IMITATER || aSeedType == SEED_IMITATER)
			{
				if (mApp->SeedTypeAvailable(aSeedType))
				{
					int aSeedX, aSeedY;
...
ZombieType AlmanacDialog::ZombieHitTest(int x, int y)
{
	...
	if (mMouseVisible && mOpenPage == AlmanacPage::ALMANAC_PAGE_ZOMBIES && y + BOARD_OFFSET_Y < cZombieRect.mY + cZombieRect.mHeight && y + BOARD_OFFSET_Y > cZombieRect.mY)
	{
		for (int i = 0; i < NUM_ALMANAC_ZOMBIES; i++)
		{
			ZombieType aZombieType = GetZombieType(i);
			if (aZombieType != ZombieType::ZOMBIE_INVALID)
			{
				...
				if (Rect(aZombieX, aZombieY, 76, 76).Contains(x, y) && y < cZombieRect.mY + cZombieRect.mHeight)
					return aZombieType;
			}
		}
	}
