...
void CutScene::PlaceAZombie(ZombieType theZombieType, int theGridX, int theGridY)
{
	...
	if (theZombieType == ZombieType::ZOMBIE_DUCKY_TUBE && mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_ZOMBOTANY_2)
	{
	...
	if (theZombieType == ZombieType::ZOMBIE_BUNGEE)
	{
		...
		if (!mBoard->StageHasRoof())		aZombie->mPosX += 100;
		aZombie->mPosY = 50.0f;
	}
	else if (theZombieType == ZombieType::ZOMBIE_BOBSLED)
	{
		aZombie->mRenderOrder = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_GROUND, 0, 1000);
		aZombie->mRow = 0;
...
void CutScene::PreloadResources()
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_PORTAL_COMBAT || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_PORTAL_COMBAT_2 || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_WEIRDMAGEDDON)
	{
	...
	if (mApp->IsStormyNightLevel() || mBoard->StageHasRain())
	{
	...
	ReanimatorEnsureDefinitionLoaded(REANIM_BUSHES3, true);
	ReanimatorEnsureDefinitionLoaded(REANIM_BUSHES4, true);
	ReanimatorEnsureDefinitionLoaded(REANIM_BUSHES5, true);
	ReanimatorEnsureDefinitionLoaded(REANIM_NIGHT_BUSHES3, true);
	ReanimatorEnsureDefinitionLoaded(REANIM_NIGHT_BUSHES4, true);
	ReanimatorEnsureDefinitionLoaded(REANIM_NIGHT_BUSHES5, true);
...
void CutScene::PlaceStreetZombies()
{
	...
	if (mApp->IsFinalBossLevel() || mApp->mGameMode == GAMEMODE_CHALLENGE_AIR_RAID ||
		mApp->mGameMode == GAMEMODE_VERSUS_PLANTS_VS_PLANTS || mApp->mGameMode == GAMEMODE_CHALLENGE_GRAVEYARD_ENCOUNTER)
		return;
	...
	if (!mApp->IsSurvivalMode() && mApp->mGameMode != GAMEMODE_CHALLENGE_ZEN_GARDEN && mApp->mGameMode != GAMEMODE_TREE_OF_WISDOM && mApp->mGameMode != GAMEMODE_INTRO && mApp->mGameMode != GAMEMODE_CHALLENGE_SQUIRREL && mApp->mGameMode != GAMEMODE_CHALLENGE_CAN_YOU_DIG_IT && mApp->mGameMode != GAMEMODE_UPSELL)
	{
		for (int aZombieType = 0; aZombieType < (int)ZombieType::NUM_ZOMBIE_TYPES; aZombieType++)
		{
			if (aZombieType != ZombieType::ZOMBIE_YETI && mBoard->mZombieAllowed[aZombieType] && !mBoard->IsZombieTypeSpawnedOnly((ZombieType)aZombieType))
			{
	...
	{
		aPreviewCapacity *= 2.0f;
		aPreviewCapacity = min(aPreviewCapacity, 18);
	}
	...
	{
		if (aZombieTypeCount[(int)aZombieType] && (Is2x2Zombie(aZombieType) || aZombieType == ZombieType::ZOMBIE_ZAMBONI) && mBoard->mZombieAllowed[aZombieType] == true)
		{
	...
	{
		FindAndPlaceZombie(ZombieType::ZOMBIE_JACK_IN_THE_BOX, aZombieGrid);
		FindAndPlaceZombie(ZombieType::ZOMBIE_LADDER, aZombieGrid);
		FindAndPlaceZombie(ZombieType::ZOMBIE_GARGANTUAR, aZombieGrid);
	}

	if (mApp->IsBungeeBlitzLevel())
	{
		int aGridX, aGridY;
		FindPlaceForStreetZombies(ZOMBIE_BUNGEE, aZombieGrid, aGridX, aGridY);
		PlaceAZombie(ZombieType::ZOMBIE_BUNGEE, 0, aGridY);
		PlaceAZombie(ZombieType::ZOMBIE_BUNGEE, 1, aGridY);
		PlaceAZombie(ZombieType::ZOMBIE_BUNGEE, 2, aGridY);
	}

	if (mBoard->mLevel == 59) {
    	FindAndPlaceZombie(ZombieType::ZOMBIE_REDEYE_GARGANTUAR, aZombieGrid);
	}
}
...
void CutScene::PlaceLawnItems()
{
	...
	if (!IsSurvivalRepick() && !mApp->IsWhackAZombieLevel())
	{
		mBoard->InitLawnMowers();
		AddFlowerPots();

		if (mBoard->mBackground == BackgroundType::BACKGROUND_GRAVEYARD) {
			mBoard->NewPlant(0, 2, SEED_FLOWERPOT);
			mBoard->NewPlant(1, 2, SEED_FLOWERPOT);
			mBoard->NewPlant(2, 2, SEED_FLOWERPOT);
		}

		if (mBoard->StageHasDarkness() && mApp->mGameMode != GAMEMODE_CHALLENGE_LIGHTS_OUT) {
			mBoard->NewPlant(2, 1, SEED_PLANTERN);
			mBoard->NewPlant(2, 3, SEED_PLANTERN);
		}

		if (mApp->mGameMode == GAMEMODE_CHALLENGE_HEAT_WAVE)
		{
			mBoard->NewPlant(0, 0, SEED_PEASHOOTER);
			mBoard->NewPlant(0, 1, SEED_PEASHOOTER);
			mBoard->NewPlant(0, 2, SEED_LILYPAD);
			mBoard->NewPlant(1, 2, SEED_LILYPAD);
			mBoard->NewPlant(0, 2, SEED_WALLNUT);
			mBoard->NewPlant(0, 3, SEED_LILYPAD);
			mBoard->NewPlant(1, 3, SEED_LILYPAD);
			mBoard->NewPlant(0, 3, SEED_WALLNUT);
			mBoard->NewPlant(0, 4, SEED_PEASHOOTER);
			mBoard->NewPlant(0, 5, SEED_PEASHOOTER);
		}

		if (mApp->mGameMode == GAMEMODE_CHALLENGE_BUTTERED_POPCORN)
		{
			mBoard->NewPlant(0, 0, SEED_COBCANNON);
			mBoard->NewPlant(0, 1, SEED_COBCANNON);
			mBoard->NewPlant(0, 4, SEED_COBCANNON);
			mBoard->NewPlant(0, 5, SEED_COBCANNON);
		}

		if (mApp->mGameMode == GAMEMODE_VERSUS_PLANTS_VS_PLANTS) {
			for (int i = 0; i < (mBoard->StageHas6Rows() ? 6 : 5); i++) {
				Plant* dummy = mBoard->NewPlant(0, i, SeedType::SEED_DUMMY, SeedType::SEED_NONE);
				dummy->mX -= 85;
				dummy->mIgnoreBlink = true;
				dummy->mIsDummy = true;

				Plant* dummyEnemy = mBoard->NewPlant(8, i, SeedType::SEED_DUMMY, SeedType::SEED_NONE);
				dummyEnemy->mX += 85;
				dummyEnemy->mIgnoreBlink = true;
				dummyEnemy->mEnemy = true;
				dummyEnemy->mIsDummy = true;
				Reanimation* aBodyReanim2 = mApp->ReanimationTryToGet(dummy->mBodyReanimID);
			}

			mBoard->NewPlant(0, 1, SeedType::SEED_SUNFLOWER, SeedType::SEED_NONE);
			mBoard->NewPlant(0, 3, SeedType::SEED_SUNFLOWER, SeedType::SEED_NONE);

			mBoard->NewPlant(8, 1, SeedType::SEED_SUNFLOWER, SeedType::SEED_NONE)->mEnemy = true;
			mBoard->NewPlant(8, 3, SeedType::SEED_SUNFLOWER, SeedType::SEED_NONE)->mEnemy = true;
		}
	}
...
void CutScene::StartLevelIntro()
{
	...
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_AIR_RAID)		mCutsceneTime = TimeReadySetPlantStart;

	float playerOffset = 0.0f;
	//if (mBoard->mIsCOOP && mBoard->HasConveyorBeltSeedBank())
	//	playerOffset = -79.0f;

	mBoard->mSeedBank->Move(SEED_BANK_OFFSET_X + playerOffset, -IMAGE_SEEDBANK->GetHeight() - BOARD_OFFSET_Y);
	mBoard->mSeedBankP2->Move(470 + playerOffset + SEED_BANK_OFFSET_X - 51, -IMAGE_SEEDBANK->GetHeight() - BOARD_OFFSET_Y );

	mBoard->mSpeedButton->mBtnNoDraw = true;
	...
	mBoard->mSeedBankP2->mCutSceneDarken = 255;
	mPlacedZombies = false;
	mPreloaded = false;
	mPlacedLawnItems = false;
	...
	else if (mApp->IsFirstTimeAdventureMode() && mBoard->mLevel == 59) {
		mCrazyDaveDialogStart = 3500;
	}
	else if (mApp->IsFirstTimeAdventureMode() && mBoard->mLevel == 51) {
	...
	if ( mBoard->mBackground == BackgroundType::BACKGROUND_GRAVEYARD || mBoard->StageHasRoof() || mBoard->StageHasDarkness() || mApp->mGameMode == GAMEMODE_CHALLENGE_POOL_PARTY || mApp->mGameMode == GAMEMODE_CHALLENGE_FLOOR_IS_LAVA || mApp->mGameMode == GAMEMODE_CHALLENGE_HEAT_WAVE || mApp->mGameMode == GAMEMODE_CHALLENGE_BUTTERED_POPCORN || mApp->mGameMode == GAMEMODE_VERSUS_PLANTS_VS_PLANTS || mApp->IsScaryPotterLevel() || mApp->IsWallnutBowlingLevel() || mApp->mGameMode == GAMEMODE_CHALLENGE_NONE_SHALL_PASS || mApp->mGameMode == GAMEMODE_CHALLENGE_BORDER_TREATY)
	{
		PreloadResources();
		PlaceLawnItems();
	}
	...
	if (mBoard->mIsVeteran && mApp->mGameMode != GameMode::GAMEMODE_INTRO)
	{
		aHouseMessage += " " + TodStringTranslate(_S("[VETERAN]"));
	}
	...
	else if (mApp->IsFinalBossLevel() || mApp->mGameMode == GAMEMODE_CHALLENGE_AIR_RAID || mApp->mGameMode == GAMEMODE_CHALLENGE_GRAVEYARD_ENCOUNTER)
	{
		mApp->mMusic->StopAllMusic();
	}
	else
	...
	if (mApp->mGameMode != GameMode::GAMEMODE_ADVENTURE) {
		mApp->UpdateDiscordRPC(TodStringTranslate(mApp->GetCurrentChallengeDef().mChallengeName));
	}
	else
	{
		int aStage = ClampInt((mBoard->mLevel - 1) / 10 + 1, 1, 6);
		int aSub = mBoard->mLevel - (aStage - 1) * 10;
...
void CutScene::CancelIntro()
{
	...
	if (mCutsceneTime > mCrazyDaveTime + TimePanLeftStart || !mBoard->ChooseSeedsOnCurrentLevel())
	{
		...
		if (!mApp->IsChallengeWithoutSeedBank())
		{
			...
			mBoard->mSeedBank->Move(SEED_BANK_OFFSET_X_END + playerOffset, -BOARD_OFFSET_Y);
			mBoard->mSeedBankP2->Move(470 + SEED_BANK_OFFSET_X_END + playerOffset - 51, -BOARD_OFFSET_Y);
		}
...
void CutScene::AddFlowerPots()
{
	...
	if (mBoard->mBackground == BACKGROUND_3_POOLWIDE)
	{
		for (int x = 0; x < 3; x++)
		{
			for (int y = 0; y < MAX_GRID_SIZE_Y; y++)
			{
				if (mBoard->CanPlantAt(x, y, SeedType::SEED_LILYPAD) == PlantingReason::PLANTING_OK)
				{
					mBoard->NewPlant(x, y, SeedType::SEED_LILYPAD, SeedType::SEED_NONE);
				}
			}
		}
		return;
	...
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_FLOOR_IS_LAVA)
	{
		for (int x = 0; x < 3; x++)
		{
			for (int y = 0; y < MAX_GRID_SIZE_Y; y++)
			{
				...
				{
					mBoard->NewPlant(x, y, SeedType::SEED_FLYINGPOT, SeedType::SEED_NONE);
				}
			}
		}
		return;
	...
	else if (mBoard->mLevel >= 43 && mBoard->mLevel <= 50 || mBoard->mLevel >= 53 && mBoard->mLevel <= 60)
	{
		aPotColumns = 3;
	}
...
void CutScene::AnimateBoard()
{
	...
	if (mCrazyDaveTime > 0)
	{
		if (mCutsceneTime == TimeEarlyDaveEnterStart)
		{
			...
			if (mApp->mGameMode == GameMode::GAMEMODE_UPSELL)
			{
				...
				aDaveReanim->SetPosition(150 + BOARD_ADDITIONAL_WIDTH, 70 + BOARD_OFFSET_Y);
			}
		}
	...
	if (mApp->mGameMode == GAMEMODE_VERSUS_PLANTS_VS_PLANTS) {
		aStreetOffset = -BOARD_ADDITIONAL_WIDTH;
	}
	...
	if (mCutsceneTime > aTimePanRightStart && mCutsceneTime <= aTimePanRightEnd){
		...
		mBoard->mRoofTreeOffset = CalcPosition(aTimePanRightStart, aTimePanRightEnd, WIDE_BOARD_WIDTH - BOARD_ADDITIONAL_WIDTH + 70, -664);
	...
	if (mBoard->ChooseSeedsOnCurrentLevel())
	{
		...
		if (mCutsceneTime > aTimeSeedChoserSlideOnStart && mCutsceneTime <= aTimeSeedChoserSlideOnEnd)
		{
			...
			aSeedChoser->mMenuButton->mY = CalcPosition(aTimeSeedChoserSlideOnStart, aTimeSeedChoserSlideOnEnd, -50 - BOARD_OFFSET_Y, -10 - BOARD_OFFSET_Y);
	...
	if (mCutsceneTime > aTimePanLeftStart)
	{
		...
		mBoard->mRoofTreeOffset = CalcPosition(aTimePanLeftStart, aTimePanLeftEnd, -664, WIDE_BOARD_WIDTH - BOARD_ADDITIONAL_WIDTH + 70);
	...
	{
		int aSeedBankY = CalcPosition(aTimeSeedBankOnStart, aTimeSeedBankOnEnd, -IMAGE_SEEDBANK->GetHeight() - BOARD_OFFSET_Y, -BOARD_OFFSET_Y);
		float playerOffset = 0.0f;
		//if (mBoard->mIsCOOP && mBoard->HasConveyorBeltSeedBank())
		//	playerOffset = -79.0f;
		
		mBoard->mSeedBank->Move(SEED_BANK_OFFSET_X_END + playerOffset, aSeedBankY);
		mBoard->mSeedBankP2->Move(470 + SEED_BANK_OFFSET_X_END + playerOffset - 51, aSeedBankY);
	}
	...
	if (mCutsceneTime >= aTimeSeedBankRightStart)
	{
		...
		int aSeedBankX = CalcPosition(aTimeSeedBankRightStart, aTimeSeedBankRightEnd, SEED_BANK_OFFSET_X_END + playerOffset, SEED_BANK_OFFSET_X + playerOffset);
		...
		mBoard->mSeedBankP2->mCutSceneDarken = aDarken;
		mBoard->mSeedBank->Move(aSeedBankX , mBoard->mSeedBank->mY);
		mBoard->mSeedBankP2->Move(470 + aSeedBankX - 51, mBoard->mSeedBank->mY);
	}
	...
	if (mSodTime > 0)
	{
		...
		if (mCutsceneTime == aTimeRollSodStart)
		{
			...
			if (mBoard->mLevel == 1)
			{
				mApp->AddReanimation(0, 0, Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ZOMBIE, 0, 0), ReanimationType::REANIM_SODROLL);
				mApp->AddTodParticle(35, 348, Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ZOMBIE, 0, 1), ParticleEffect::PARTICLE_SOD_ROLL);
			}
			else if (mBoard->mLevel == 2)
			{
				mApp->AddReanimation(0, -102, Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ZOMBIE, 0, 0), ReanimationType::REANIM_SODROLL);
				mApp->AddReanimation(0, 111, Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ZOMBIE, 0, 0), ReanimationType::REANIM_SODROLL);
				mApp->AddTodParticle(35, 246, Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ZOMBIE, 0, 1), ParticleEffect::PARTICLE_SOD_ROLL);
				mApp->AddTodParticle(35, 459, Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ZOMBIE, 0, 1), ParticleEffect::PARTICLE_SOD_ROLL);
			}
			else if (mBoard->mLevel == 4)
			{
				...
				mApp->AddTodParticle(32, 150, Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ZOMBIE, 0, 1), ParticleEffect::PARTICLE_SOD_ROLL);
				mApp->AddTodParticle(32, 511 + 21, Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ZOMBIE, 0, 1), ParticleEffect::PARTICLE_SOD_ROLL);
			}
		}
	...
	if (mBossTime > 0)
	{
		...
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_AIR_RAID)
		{
			...
			if (aBoss)
				aBoss->mPosX = max(TimeReadySetPlantStart + mBossTime - 20 - mCutsceneTime, 0);
	...
	// ▲ 僵王博士关卡背景音乐的播放
	// ====================================================================================================
	int targetTime = aTimeSeedBankOnStart;
	
	if (mApp->mGameMode == GAMEMODE_CHALLENGE_GRAVEYARD_ENCOUNTER)
		targetTime = aTimePanLeftEnd + mBossTime;

	if ((mApp->IsFinalBossLevel() || mApp->mGameMode == GAMEMODE_CHALLENGE_AIR_RAID || mApp->mGameMode == GAMEMODE_CHALLENGE_GRAVEYARD_ENCOUNTER) && mCutsceneTime == targetTime)
	{
		mApp->mMusic->StartGameMusic();
	}
...
void CutScene::ShowShovel()
{
	if (mApp->IsWhackAZombieLevel() ||
		...
		mApp->IsIZombieLevel() || mApp->mGameMode == GAMEMODE_VERSUS_PLANTS_VS_PLANTS)
		return;
...
void CutScene::StartSeedChooser()
{
	...
	if (mApp->mGameMode != GameMode::GAMEMODE_ADVENTURE) {
		mApp->UpdateDiscordRPC(TodStringTranslate(mApp->GetCurrentChallengeDef().mChallengeName), "Choosing Seeds");
	}
	else
	{
		int aStage = ClampInt((mBoard->mLevel - 1) / 10 + 1, 1, 6);
		...
		std::string Details = "Adventure " + std::to_string(aStage) + "-" + std::to_string(aSub);
		const char* DetailsChar = Details.c_str();
		mApp->UpdateDiscordRPC(DetailsChar, "Choosing Seeds");
	}

	/*for (int i = 0; i < SEEDBANK_MAX; i++) {
		if (!mApp->mPlayerInfo->mLastSeedsInBank[i])	break;
		bool isPickedByDave = false;
		for (SeedType aSeedType = SEED_PEASHOOTER; aSeedType < NUM_SEEDS_IN_CHOOSER; aSeedType = (SeedType)(aSeedType + 1)) {
			if (mApp->SeedTypeAvailable(aSeedType)) {
				...
				if (aChosenSeed.mSeedState == SEED_IN_BANK && aChosenSeed.mSeedIndexInBank == i && aChosenSeed.mCrazyDavePicked) {
					isPickedByDave = true;
					break;
				}
			}
		...
		if (isPickedByDave)	continue;

		ChosenSeed& aChosenSeed = mApp->mSeedChooserScreen->mChosenSeeds[mApp->mPlayerInfo->mLastSeedsInBank[i]];

		if (aChosenSeed.mCrazyDavePicked)	continue;

		aChosenSeed.mTimeStartMotion = mApp->mSeedChooserScreen->mSeedChooserAge;
		aChosenSeed.mTimeEndMotion = mApp->mSeedChooserScreen->mSeedChooserAge + 15;
		aChosenSeed.mStartX = aChosenSeed.mX;
		aChosenSeed.mStartY = aChosenSeed.mY;
		mApp->mSeedChooserScreen->GetSeedPositionInBank(i, aChosenSeed.mEndX, aChosenSeed.mEndY);
		aChosenSeed.mSeedState = SEED_FLYING_TO_BANK;
		aChosenSeed.mSeedIndexInBank = i;
		mApp->mSeedChooserScreen->mSeedsInFlight++;
		mApp->mSeedChooserScreen->mSeedsInBank++;
		mApp->mSeedChooserScreen->EnableStartButton(true);
	}*/
...
void CutScene::EndSeedChooser()
{
	...
	if (mApp->mGameMode != GameMode::GAMEMODE_ADVENTURE) {
		mApp->UpdateDiscordRPC(TodStringTranslate(mApp->GetCurrentChallengeDef().mChallengeName), "Starting");
	}
	else
	{
		int aStage = ClampInt((mBoard->mLevel - 1) / 10 + 1, 1, 6);
		...
		std::string Details = "Adventure " + std::to_string(aStage) + "-" + std::to_string(aSub);
		const char* DetailsChar = Details.c_str();
		mApp->UpdateDiscordRPC(DetailsChar, "Starting");
	}
}
...
void CutScene::Update()
{
	...
	if (!aCutsceneTimeStop)
	{
		...
		if (mApp->mGameMode == GAMEMODE_CHALLENGE_AIR_RAID)		
		{
			mBoard->mChallenge->mAirRaidCLOUDTime = (mBoard->mChallenge->mAirRaidCLOUDTime + 3845) % 3840;
			mBoard->mChallenge->mAirRaidSTARTime = (mBoard->mChallenge->mAirRaidSTARTime + 3841) % 3840;
		}
	...
	if (mCutsceneTime >= aTimeStart)
	{
		...
		if (mBoard->mSpeedButton && mApp->mGameMode != GAMEMODE_CHALLENGE_ZEN_GARDEN && mApp->mGameMode != GAMEMODE_TREE_OF_WISDOM && mApp->mGameMode != GAMEMODE_VERSUS_PLANTS_VS_PLANTS)
		{
			mBoard->mSpeedButton->mBtnNoDraw = false;
	...
	if (!IsSurvivalRepick() && mBoard->mLevel != 5)
	{
		Plant* aPlant = nullptr;
		while (mBoard->IteratePlants(aPlant)) {
			aPlant->Animate();
...
void CutScene::UpdateZombiesWon()
{
	...
	if (mCutsceneTime == LostTimeBrainGraphicStart)
	{
		...
		//mApp->UpdateDiscordRPC(TodStringTranslate(mApp->GetCurrentChallengeDef().mChallengeName) + (string)" - Game Over", "THE ZOMBIES ATE YOUR BRAINS!");
	}
	...
	if (mCutsceneTime == LostTimeEnd)
	{
		if (mApp->IsSurvivalMode())
		{
			...
			mApp->UpdateDiscordRPC(TodStringTranslate(mApp->GetCurrentChallengeDef().mChallengeName), aStr);
		}
		else
		{
			...
			/*if (mApp->mGameMode != GAMEMODE_ADVENTURE) {
				...
				std::string Details = "Adventure " + std::to_string(aStage) + "-" + std::to_string(aSub);
				const char* DetailsChar = Details.c_str();
				mApp->UpdateDiscordRPC(DetailsChar + (string)" - Game Over", "THE ZOMBIES ATE YOUR BRAINS!");
			}*/
		}
	}
}
...
void CutScene::AdvanceCrazyDaveDialog(bool theJustSkipping)
{
	...
	if (!mApp->AdvanceCrazyDaveText())
	{
		...
		{
			if (mBoard->ChooseSeedsOnCurrentLevel() && mApp->mGameMode != GAMEMODE_CHALLENGE_GRAVEYARD_ENCOUNTER)
			{
				mApp->mMusic->MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_CHOOSE_YOUR_SEEDS);
			}
...
void CutScene::ClearUpsellBoard()
{
	...
	mBoard->mPlayerController.DataArrayDispose();
	...
	Bush* aBush = nullptr;
	while (mBoard->IterateBushes(aBush))
	{
		mApp->ReanimationGet(aBush->mReanimID)->ReanimationDie();
	}
...
void CutScene::UpdateUpsell()
{
	...
	if (mCrazyDaveLastTalkIndex == -1 && mCrazyDaveLastTalkIndex != mCrazyDaveDialogStart)
	{
		mApp->CrazyDaveTalkIndex(mCrazyDaveDialogStart);
...
{
	mBoard->Move(TodAnimateCurve(TimeIntro_PanRightStart, TimeIntro_PanRightEnd, mCutsceneTime, -100 , 100, TodCurves::CURVE_LINEAR) + BOARD_ADDITIONAL_WIDTH, BOARD_OFFSET_Y);
...
void CutScene::DrawIntro(Graphics* g)
{
	...
	if (mCutsceneTime == TimeIntro_LogoStart)
		mBoard->AnimateBush(1);
