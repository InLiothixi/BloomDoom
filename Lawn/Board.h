...
public:
	LawnApp*						mApp;													//+0x8C
	DataArray<Plant>				mExtraPlants;
	DataArray<PlayerController>     mPlayerController;
	PlayerController*				mPlayers[4];
	DataArray<Zombie>				mZombies;												//+0x90
	DataArray<Bush>					mBushes;
	DataArray<BushCheap>			mBushesCheap;
	Bush*							mBushList[6];
	BushCheap*						mBushCheapList[6];
	...
	int								mFogGridCelLook[MAX_GRID_SIZE_X + 4][MAX_GRID_SIZE_Y];
	int								mGridCelOffset[MAX_GRID_SIZE_X][MAX_GRID_SIZE_Y][2];	//+0x318
	int								mGridCelFog[MAX_GRID_SIZE_X][MAX_GRID_SIZE_Y + 4];		//+0x4C8
	int								mFogGridCelFog[MAX_GRID_SIZE_X + 4][MAX_GRID_SIZE_Y + 4];
	...
	ParticleSystemID				mPoolSparklyParticleIDExtra2;
	ParticleSystemID				mPoolSparklyParticleIDExtra3;
	ReanimationID					mFwooshID[MAX_GRID_SIZE_Y][12];							//+0x5624
	int								mFwooshGrid[12][MAX_GRID_SIZE_Y];
...
public:
	...
	Plant*							AddPlantEnemy(int theGridX, int theGridY, SeedType theSeedType, SeedType theImitaterType = SeedType::SEED_NONE);
	...
	Zombie*							AddZombie(ZombieType theZombieType, int theFromWave, bool introBush);
	...
	void							TryToSaveGame();
	void							SaveCurrentTime(std::string value);
	...
	bool							StageHasPool();
	/*inline*/ bool					StageHasHighGrounds();
	/*inline*/ bool					StageHas6Rows();
	/*inline*/ bool					StageHasFog();
	bool							StageHasDarkness();
	bool							StageHasRain();
	...
	void							GetPlantsOnLawn2(int theGridX, int theGridY, PlantsOnLawn* thePlantOnLawn);
	...
	bool							IterateBushesCheap(BushCheap*& theBushCheap);
	...
	Plant*							GetFlyingPotAt(int theGridX, int theGridY);
	...
	void							KillAllPlantsInRadius(int theRow, int theX, int theY, int theRadius, int theRowRange, bool theBurn, int theDamageRangeFlags, bool mEnemy);
	...
	void							DamageAllZombieInRadius(int theRow, int theX, int theY, int theRadius, int theRowRange, bool theBurn, int theDamageRangeFlags, int daDamage);
	...
	Plant*							FindPlanternPlant(int gridX, int gridY, int range = 2);
	...
	Plant*							NewPlantEnemy(int theGridX, int theGridY, SeedType theSeedType, SeedType theImitaterType = SeedType::SEED_NONE);
	...
	void							ClearFogAroundPoint(int gridX, int gridY, int theSize, int speed);
	...
	void							UpdateGameRPC(SexyString name, SexyString info);
