...
void Coin::CoinInitialize(int theX, int theY, CoinType theCoinType, CoinMotion theCoinMotion)
{
    ...
    mUsableImitaterSeedType = SeedType::SEED_NONE;
    mNeedsBouncyArrow = false;
    mHasBouncyArrow = false;
    mHitGround = false;
    for (int i = 0; i < 4; i++) {
        mAttractedToPlayer[i] = false;
    }
    mWasAttracted = false;
    mTimesDropped = 0;
    ...
    else if (mType == CoinType::COIN_TROPHY || mType == CoinType::COIN_DIAMOND_TROPHY)
    {
        mWidth = IMAGE_TROPHY->GetCelWidth();
        mHeight = IMAGE_TROPHY->GetCelHeight();
        mRenderOrder = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_ABOVE_UI, 0, 0);
    }
    else if (mType == CoinType::COIN_AWARD_SILVER_SUNFLOWER || mType == CoinType::COIN_AWARD_GOLD_SUNFLOWER || mType == CoinType::COIN_AWARD_DIAMOND_SUNFLOWER)
    {
    ...
    case CoinMotion::COIN_MOTION_COIN:
        ...
        if (mType == CoinType::COIN_AWARD_SILVER_SUNFLOWER || mType == CoinType::COIN_AWARD_GOLD_SUNFLOWER || mType == CoinType::COIN_AWARD_DIAMOND_SUNFLOWER)
        {
            mPosY -= 100.0f;
        ...
        if (mType == CoinType::COIN_FINAL_SEED_PACKET ||
            ...
            mType == CoinType::COIN_DIAMOND_TROPHY)
        {
            mGroundY -= 30;
        }
        break;
...
void Coin::UpdateFall()
{
    ...
    else if (mPosY + mVelY < mGroundY)
    {
        ...
        if (!mWasAttracted)
        {
            ...
            else if (mPosX < 0.0f && mApp->mGameMode != GAMEMODE_CHALLENGE_WINDNESDAY)
            {
                mPosX = 0.0f;
                mVelX = 0.4f + RandRangeFloat(0.0f, 0.4f);
            }
    ...
    {
        if (mApp->mGameMode == GAMEMODE_CHALLENGE_WINDNESDAY && (IsMoney() || mType == COIN_SUN || mType == COIN_SMALLSUN || mType == COIN_LARGESUN))
        {
            mPosX -= 3.33f * 0.25f;
        }
        ...
        if (mNeedsBouncyArrow && !mHasBouncyArrow)
        {
            ...
            if (mType == CoinType::COIN_TROPHY || mType == CoinType::COIN_DIAMOND_TROPHY)
            {
        ...
        if (!mApp->IsLastStandLevel(mApp->mGameMode) || mBoard == nullptr ||
...
void Coin::UpdateCollected()
{
    ...
    if (IsSun())
    {
        aDestX = 5 - BOARD_ADDITIONAL_WIDTH;
        aDestY = 0 - BOARD_OFFSET_Y;
    }
    else if (IsMoney())
    {
        aDestX = 39 - BOARD_ADDITIONAL_WIDTH;
        aDestY = 558 + BOARD_OFFSET_Y;
...
{
    mCoinAge++;
    if (mApp && mApp->mGameScene != GameScenes::SCENE_PLAYING && mApp->mGameScene != GameScenes::SCENE_AWARD && mBoard && !mBoard->mCutScene->ShouldRunUpsellBoard())
    {
        return;
    }
    ...
    else if (!mIsBeingCollected)
    {
        ...
        if (mBoard && mApp->mGameMode != GAMEMODE_CHALLENGE_ZEN_GARDEN && mApp->mGameMode != GAMEMODE_TREE_OF_WISDOM && (IsSun() || IsMoney()) && mAllowAttraction)
        {
            for (int i = 0; i < mBoard->mMaxPlayers; i++) {
                PlayerController* player = mBoard->mPlayers[i];

                float posX = player->mX;
                float posY = player->mY;
                if (player->mControlType == ControlType::CONTROL_MOUSE) {
                    posX = mApp->mWidgetManager->mLastMouseX - BOARD_ADDITIONAL_WIDTH;
                    posY = mApp->mWidgetManager->mLastMouseY - BOARD_OFFSET_Y;
                }

                bool willAttract = player->mControlType != ControlType::CONTROL_MOUSE || mApp->mWidgetManager->IsLeftButtonDown();

                float dX = posX - mPosX - mWidth / 2.0f;
                float dY = posY - mPosY - mHeight / 2.0f;
                float distance = sqrt(dX * dX + dY * dY);

                if (mAttractedToPlayer[i]) {
                    float dir = atan2(posY - mPosY - mHeight / 2.0f, posX - mPosX - mWidth / 2.0f);
                    mVelX = cos(dir) * 10;
                    mVelY = sin(dir) * 10;

                    if (mHitGround) mGroundY = mPosY + 20.f;
                }

                if (distance <= 30 && mAttractedToPlayer[i]) {
                    PlayCollectSound();
                    Collect();
                ...
                else if (distance <= 115 && willAttract && !mWasAttracted) {
                    mAttractedToPlayer[i] = true;
                    mWasAttracted = true;
                }
...
void Coin::Draw(Graphics* g)
{
    ...
    else if (mType == CoinType::COIN_DIAMOND_TROPHY)
    {
        aImage = IMAGE_DIAMOND_TROPHY_HI_RES;
        aOffsetX -= mWidth / 2;
        aOffsetY -= mHeight / 2;
        aDrawScale *= 0.5f;
    }
    ...
    else if (mType == CoinType::COIN_AWARD_DIAMOND_SUNFLOWER)
    {
        aImage = IMAGE_DIAMOND_SUNFLOWER_TROPHY;
        aImageCelCol = 1;
        aOffsetX -= 5.0f;
        aDrawScale *= 0.6f;
    }
    ...
    else if (mType == CoinType::COIN_USABLE_SEED_PACKET)
    {
        ...
        g->SetColorizeImages(true);
        DrawSeedPacket(g, (int)mPosX, (int)mPosY, mUsableSeedType, mUsableImitaterSeedType, 0.0f, aGrayness, false, false);
        g->SetColorizeImages(false);
    ...
    g->SetColorizeImages(true);
    if (mType == CoinType::COIN_AWARD_DIAMOND_SUNFLOWER)
    {
        TodDrawImageCenterScaledF(g, aImage, mPosX + aOffsetX, mPosY + aOffsetY, aDrawScale, aDrawScale);
    }
    else
    {
...
void Coin::Collect()
{
    ...
    if (IsLevelAward())
    {
        ...
        {
            if (mType == CoinType::COIN_TROPHY || mType == CoinType::COIN_DIAMOND_TROPHY || mType == CoinType::COIN_AWARD_MONEY_BAG)
            {
                mApp->PlayFoley(FoleyType::FOLEY_COIN);
                FanOutCoins(CoinType::COIN_GOLD, 5);
            }
        ...
        {
            FanOutCoins(CoinType::COIN_DIAMOND, 10);
        }
        else if (mType == CoinType::COIN_AWARD_GOLD_SUNFLOWER)
        {
        ...
        {
            FanOutCoins(CoinType::COIN_DIAMOND, 2);
        }
        else if (mType == CoinType::COIN_TROPHY)
        {
            mApp->PlaySample(SOUND_DIAMOND);
        ...
        {
            mApp->PlayFoley(FoleyType::FOLEY_COIN);
            FanOutCoins(CoinType::COIN_GOLD, (mBoard->mIsVeteran ? 15 : 5));
        }
        else
        {
    ...
    if (mType == CoinType::COIN_USABLE_SEED_PACKET)
    {
        ...
        mBoard->mCursorObject->mImitaterType = mUsableImitaterSeedType;
...
{
    if (mApp->mGameScene != SCENE_PLAYING) return false;
...
bool Coin::IsLevelAward()
{
    return
        ...
        mType == CoinType::COIN_AWARD_DIAMOND_SUNFLOWER ||
        mType == CoinType::COIN_SHOVEL ||
        mType == CoinType::COIN_CARKEYS ||
        mType == CoinType::COIN_ALMANAC ||
        mType == CoinType::COIN_VASE ||
